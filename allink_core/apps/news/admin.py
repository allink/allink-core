...
class KnowledgeContentAdminForm(AllinkCategoryAdminForm):

    def __init__(self, *args, **kwargs):
        super(KnowledgeContentAdminForm, self).__init__(*args, **kwargs)
...
class KnowledgeAdmin(AllinkMediaAdminMixin, AllinkSEOAdminMixin, AllinkCategoryAdminMixin,
                AllinkTeaserAdminMixin, PlaceholderAdminMixin,
                TranslatableAdmin):
    form = KnowledgeContentAdminForm
    list_display = ('title', 'status', 'all_categories_column','entry_date',)
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        if db_field.name == 'lead':
            ...
            kwargs['max_length'] = 400
            kwargs['help_text'] = 'Max. 400 characters.'
        return super(KnowledgeAdmin, self).formfield_for_dbfield(db_field, **kwargs)
