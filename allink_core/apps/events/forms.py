...
#
#         }
#         fields = ('event', 'salutation', 'first_name', 'last_name', 'company_name', 'email', 'phone',
#         'message', 'terms', 'terms_accepted')
#
#     def __init__(self, *args, **kwargs):
#         super(EventsRegistrationTermsForm, self).__init__(*args, **kwargs)
#         try:
#             self.fields['terms_accepted'].label = mark_safe(_('I have read and accept the <a href="%s"
#             target="_blank">terms and conditions.</a>')) % (AllinkTerms.objects.get_published().terms_
#             cms_page.get_absolute_url())
#             self.fields['terms'].initial = AllinkTerms.objects.get_published()
#         except:
#             raise AttributeError(_('Please configure Terms. And create the corresponding cms Page.'))
