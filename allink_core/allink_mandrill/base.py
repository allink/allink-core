...
from allink_core.allink_mandrill.helpers import check_result_status
...
class AllinkMandrillEmailBase:
    ...
    @staticmethod
    def create_email_entry(email_address, name, type):
        return {
            'email': email_address,
            'name': name,
            'type': type
    ...
    def _fetch_to_email_adresses_dev_mode(self):
        ...
        """
        return [self.create_email_entry(email, 'allink Test', 'to') for email in settings.ALLINK_MANDRILL_DEV_MODE_TO_EMAIL_ADDRESSES]
