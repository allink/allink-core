...
def migrate_links(apps, schema_editor):
    AllinkImagePlugin = apps.get_model("djangocms_image", "AllinkImagePlugin")
    Page = apps.get_model("cms", "Page")

    link_list = get_link_list(apps)
    print()

    for l in AllinkImagePlugin.objects.filter(link_internal__isnull=False).exclude(link_internal=''):
